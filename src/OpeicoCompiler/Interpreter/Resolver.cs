...
namespace OpeicoCompiler.Interpreter
{
    ...
    {
        private readonly OpeicoInterpreter interpreter;
        private FunctionType currentFunction = FunctionType.NONE;
        private ClassType currentClass = ClassType.NONE;
        private readonly ServiceProvider? ServiceProvider;
        private readonly Stack<Dictionary<string, bool>> scopes = new();
        private readonly Dictionary<string, BlockScope?> processScope = new();
        ...
        internal Resolver(OpeicoInterpreter interpreter)
        {
            this.interpreter = interpreter;

            ServiceProvider = interpreter.ServiceProvider;
            if (ServiceProvider != null)
            {
                compilerError = ServiceProvider?.GetService<ICompilerError>();
            }
        }
        ...
        public object VisitAssignExpr(Expr.Assign expr)
        {
            ...
            {
                if (value != null && value.lexemeScope.TryGetValue(expr.ExpressionLexeme?.ToString()!, out _))
                {
        ...
        public object VisitClassStmt(Stmt.Class stmt)
        {
            ...
            {
                compilerError?.AddError("can't inhert from itself");
            }
        ...
        public object VisitReturnStmt(Stmt.Return stmt)
        {
            ...
            {
                compilerError?.AddError("Can't reach return. No function defined");
            }
            ...
            if (stmt.expr != null)
            {
                ...
                {
                    compilerError?.AddError("Can't return from an initializer function");
                }
        ...
        public object VisitVariableExpr(Expr.Variable expr)
        {
            try
            {
                ...
                string currentScope = blockScope.Peek();

                if (processScope.TryGetValue(currentScope, out BlockScope? localScope))
                {
                    if (localScope != null && localScope.lexemeScope.TryGetValue(expr.ExpressionLexeme!.ToString(), out Lexeme? value))
                    {
            ...
            {
                compilerError?.AddError(ex.Message);
            }
        ...
        private void Declare(Lexeme name)
        {
            if (blockScope != null && blockScope.Count > 0)
            {
                ...
                if (processScope.TryGetValue(block, out BlockScope? bsLocal))
                {
                ...
                processScope.Add(block, bsObject);
            }
        }
        ...
        private void ResolveFunction(Stmt.Function function, FunctionType type)
        {
            ...
            foreach (Parse.TypeParameterMap param in function.typeParameterMaps)
            {
                if (param.parameterName is not Expr.Variable literalName)
                {
